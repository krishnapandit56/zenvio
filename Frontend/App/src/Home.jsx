...
export default function Home() {
  ...
  async function fetchsearch(query) {
    const result = await fetch(
      `https://zenvio-h5be.onrender.com/searchproduct?searchtext=${encodeURIComponent(query)}`,
      {
        method: "post",
        credentials: "include",
        headers: {
  ...
  return (
    <div className="min-h-screen max-w-screen  bg-gray-50 overflow-x-hidden overflow-y-auto">
      ...
      <div className="hidden md:block flex md:w-[380px] md:flex space-x-1 flex-shrink-0 fixed top-0 right-1 h-[72px] ">
        ...
        <button
          onClick={() => {
            Navigate("/YourOrders", { state: username });
          }}
          className="w-[80px] md:w-[100px] flex items-center md:px-0 py-0 !bg-orange-200 text-gray-700 !rounded-none hover:!bg-orange-100 transition"
        >
          Your Orders
        </button>
        ...
        <button
          ...
          className="w-[80px] md:w-[100px] flex items-center md:px-0 py-0 !bg-orange-300 text-gray-700 !rounded-none hover:!bg-orange-100 transition"
        >
          Logout
        </button>
      </div>
      ...
      <div className="relative top-[100px] text-[10px] md:text-sm md:relative pb-4 md:w-[350px] md:w-screen h-[601.5px] bg-white overflow-auto scroll-smooth px-4">
        ...
        {searchtext.length === 0 && (
          <div>
            <h2 className="text-gray-700 text-3xl font-semibold flex">
        ...
        {searchtext.length === 0 &&
          (Array.isArray(recentarray) ? recentarray : []).map((element) => (
            <div
              key={element._id}
